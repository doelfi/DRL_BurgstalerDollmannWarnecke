...
# We believe that our code is correct but after running for a certain amount of time we get a "BrokenPipe Error" on multiple computers. 
# We tried fixing the code itself, adapting the hyperparameters and asked other groups for their solution, but we could not solve this one.
# Additionally, the YT series "DQN from scratch" STILL does not have the second-to-last video uploaded where several bugfixes should be applied
# (as the code from the vide beforehand is not running and the last video is only showing results and optimizations).
# We implemented a Double DQN with polyak averaging that we think is working as intended.
...
def train_dqn(train_dqn_network, target_network, dataset, optimizer, gamma : float, num_training_steps : int, use_double : bool = False):
...
def test_q_network(test_dqn_network, environment_name : str, num_parallel_tests : int, gamma : float, epsilon : float = 0.2):
    ...
    while not done:
        ...
        random_actions = tf.random.uniform(shape=(num_parallel_tests,), 
                                           minval=0, 
                                           maxval=num_possible_actions, 
                                           dtype=tf.int64) # Tensor of type tf.int64, shape(parrallel_game_unrolls,)
        epsilon_sampling = tf.random.uniform(shape=(num_parallel_tests,),
                                             minval=0,
                                             maxval=1,
                                             dtype=tf.float32) > epsilon
        actions = tf.where(epsilon_sampling, greedy_actions, random_actions).numpy()
